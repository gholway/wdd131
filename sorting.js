...
window.onload = function() {
    // Display original simpleList
    displayResult("originalSimpleList", [...simpleList]); // Use spread to show original before sorting

    const simpleSort = [...simpleList].sort(); // Use spread to avoid modifying original simpleList for this display
    displayResult("simpleSortResult", simpleSort);
    ...
    const anotherSort = [...simpleList].sort(compareFn); // Use spread to avoid modifying original simpleList for this display
    displayResult("anotherSortResult", anotherSort);
    ...
    displayResult("searchBResult", searchList(simpleList, "b"));
    displayResult("searchAnResult", searchList(simpleList, "an"));
    ...
    function searchListObjects(list, q) {
        ...
        // Function to extract numerical distance from string (e.g., "3 miles" -> 3)
        function getNumericDistance(distanceStr) {
            return parseFloat(distanceStr.split(' ')[0]);
        }

        const sorted = filtered.sort((a, b) => {
            const distanceA = getNumericDistance(a.distance);
            const distanceB = getNumericDistance(b.distance);
            return distanceA - distanceB; // Ascending sort by distance
        });
        return sorted;
    }
    displayResult("searchYellowstoneResult", searchListObjects(hikes, "yellowstone"));
    displayResult("searchModerateResult", searchListObjects(hikes, "moderate"));
    displayResult("searchAlResult", searchListObjects(hikes, "al"));
};